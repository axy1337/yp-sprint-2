...
CREATE TABLE IF NOT EXISTS dwh.customer_report_datamart (
    ...
    customer_birthdate DATE NOT NULL, -- дата рождения заказчика
    customer_email VARCHAR NOT NULL, -- электронная почта заказчика
    total_spent NUMERIC(15, 2) NOT NULL, -- сумма, которую потратил заказчик
    platform_earnings NUMERIC(15, 2) NOT NULL, -- сумма, которую заработала платформа от покупок заказчика за месяц (10% от суммы, которую потратил заказчик)
    orders_count BIGINT NOT NULL, -- количество заказов у заказчика за месяц
    avg_order_cost NUMERIC(15, 2) NOT NULL, -- средняя стоимость одного заказа у заказчика за месяц
    median_order_completion_time NUMERIC(10, 1), -- медианное время в днях от момента создания заказа до его завершения за месяц
    popular_product_category VARCHAR NOT NULL, -- самая популярная категория товаров у этого заказчика за месяц
    popular_craftsman_id BIGINT, -- идентификатор самого популярного мастера ручной работы у заказчика
    orders_created_count BIGINT NOT NULL, -- количество созданных заказов за месяц
    orders_in_progress_count BIGINT NOT NULL, -- количество заказов в процессе изготовки за месяц
    orders_in_delivery_count BIGINT NOT NULL, -- количество заказов в доставке за месяц
    orders_completed_count BIGINT NOT NULL, -- количество завершённых заказов за месяц
    orders_not_completed_count BIGINT NOT NULL, -- количество незавершённых заказов за месяц
    reporting_period VARCHAR NOT NULL, -- отчётный период, год и месяц
